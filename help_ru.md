...
* **Способ 2 (Через ссылку):** Нажмите кнопку **«Поделиться»**, которая находится прямо под сгенерированным QR-кодом. Телеграм предложит вам ![Поделиться визиткой](images/share_ru.png) выбрать чат с конкретным пользователем, с которым вы хотите поделиться ссылкой. При переходе по этой ссылке у него также откроется Телеграм с вашей визиткой.
